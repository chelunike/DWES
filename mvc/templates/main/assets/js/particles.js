	...
	function() {
		function t() {
			u = window.innerWidth, c = window.innerHeight, g = {
				x: u / 2,
				y: c / 2
			}, p = document.getElementById('top'), p.style.height = c + "px", f = document.getElementById("canvas"), f.width = u, f.height = c, m = f.getContext("2d"), d = [];
		...
		function h(t) {
			if (t.active)
				for (var e in t.closest) m.beginPath(), m.moveTo(t.x, t.y), m.lineTo(t.closest[e].x, t.closest[e].y), m.strokeStyle = "rgba(255,266,255," + t.active + ")", m.stroke()
		}
